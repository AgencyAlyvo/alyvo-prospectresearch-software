<template>
  <section class="grid gap-4 rounded-lg border border-[#2f3d67] bg-[#0b1433]/70 p-5 md:grid-cols-3">
    ...
    <ProspectCommercialEditModal v-model="editOpen" :prospect="prospect" />
  </section>
</template>
...
/**
 * Props du bloc suivi commercial.
 */
type ProspectCommercialTrackingProps = {
...
const props: ProspectCommercialTrackingProps = defineProps<ProspectCommercialTrackingProps>()
...
const trackingItems: ComputedRef<TrackingItem[]> = computed((): TrackingItem[] => {
  ...
  items.push({
    label: 'Notes',
    value: props.prospect.identifiedNeed,
    multiline: true,
    wide: true,
  })
